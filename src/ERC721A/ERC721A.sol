...
contract ERC721A is IERC721A {
    ...
    function _mint(address to, uint256 quantity) internal virtual {
        ...
        _beforeTokenTransfers(address(0), to, startTokenId, quantity);  //  原始為空
        ...
        unchecked {
            ...
            // Mask `to` to the lower 160 bits, in case the upper bits somehow aren't clean.
            // uint256 _BITMASK_ADDRESS_DATA_ENTRY = (1 << 64) - 1; → 0xFFFFFFFFFFFFFFFF
            // 將 to 擴展成 uint256，並且只保留 uint160(20bytes) 的值，高位補 0
            uint256 toMasked = uint256(uint160(to)) & _BITMASK_ADDRESS;

            if (toMasked == 0) _revert(MintToZeroAddress.selector); // to 為 0, 則 revert

            uint256 end = startTokenId + quantity;
            uint256 tokenId = startTokenId;

            // emit transfert event，記錄 _currentInedx ~ end 的 token,
            // transfer 給 to
            do {
                assembly {
                    // Emit the `Transfer` event.
                    log4(
                        0, // Start of data (0, since no data).
                        0, // End of data (0, since no data).
                        _TRANSFER_EVENT_SIGNATURE, // Signature.    直接將 function signature(keccak256(bytes("Transfer(address,address,uint256)"))) 寫成 constant
                        0, // `address(0)`.
                        toMasked, // `to`.
                        tokenId // `tokenId`.
                    )
            ...
            _currentIndex = end;  // 下次 mint 從這個編號開始
        }
        _afterTokenTransfers(address(0), to, startTokenId, quantity);   //  原始為空
    }
